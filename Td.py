...
class TDAgent:
    ...
    def update(self, state, action, reward, next_state):
        # TD Update
        td_target = reward + self.discount * np.max(self.q_values[next_state])
    ...
    def save_q_values(self, filename='q_values.pkl'):
        with open(filename, 'wb') as f:
            pickle.dump(self.q_values, f)  # Corrected from self.q_table to self.q_values

    def load_q_values(self, filename='q_values.pkl'):
        try:
            with open(filename, 'rb') as f:
                self.q_values = pickle.load(f)  # Corrected from self.q_table to self.q_values
...
def train_agent(n_episodes):
    ...
    agent = TDAgent()
    scores = []
    food_counts = []  # List to store food counts per episode
    ...
    mean_food = sum(food_counts) / n_episodes  # Average food count
